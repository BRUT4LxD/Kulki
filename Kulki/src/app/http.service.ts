...
export class HttpService {
  ...
  private gamesTop100ByDateUrl = 'http://localhost:8080/gamesByDate';
  private timeUrl = 'http://worldclockapi.com/api/json/utc/now';
  private settingsUrl = '/assets/settings.json';
  private timeSpentPlayingUrl = 'http://localhost:8080/games/time';
  ...
  public getTop100GamesByDate(): Observable<Game[]> {
    return this.http.get<Game[]>(`${this.gamesTop100ByDateUrl}`);
  }
  public getTimeSpentPlaying(id: string): Observable<string> {
    return this.http.get<string>(`${this.timeSpentPlayingUrl}/${id}`);
  }
}
